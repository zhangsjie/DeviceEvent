...
public class VerticaInputSplit extends InputSplit implements Writable {
  ...
  VerticaConfiguration1 vtconfig = null;
  ...
  public void configure(Configuration conf) throws Exception {
    ...
    vtconfig = new VerticaConfiguration1(conf);
  ...
  public Configuration getConfiguration() {
	...
	public void readFields(DataInput in) throws IOException {
		...
		if (paramCount > 0) {
    		...
    		for (int i = 0; i < paramCount; i++) {
				segmentParams.add(VerticaRecord1.readField(type, in));
			}
		}
		start = in.readLong();
		end = in.readLong();
	...
	public void write(DataOutput out) throws IOException {
		...
		if (segmentParams != null && segmentParams.size() > 0) {
			...
			int type = VerticaRecord1.getType(segmentParams.get(0));
			out.writeInt(type);
			for (Object o : segmentParams)
				VerticaRecord1.write(o, type, out);
		} else {
