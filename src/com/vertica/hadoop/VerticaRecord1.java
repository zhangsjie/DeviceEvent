 ...
 * 
 */
public class VerticaRecord1 implements Writable {
	...
	Vector<Integer> columnsizes=null;
	HashMap<String,Integer> columnsizeMap=null;
	/**
	 * Default constructor should NOT be invoked by customers.
	 * Available mainly for Hadoop framework.
	 */ 
	public VerticaRecord1() {
		...
		columnsizes = new Vector<Integer>();
		columnsizeMap = new HashMap<String, Integer>();
	}
	...
	public VerticaRecord1(Configuration conf) 
		throws ClassNotFoundException, SQLException, IOException {
		VerticaConfiguration1 config = new VerticaConfiguration1(conf);
		...
		String metaStmt = "select ordinal_position, column_name, data_type, is_identity, data_type_name,column_size " +
		...
		while (rs.next()) {
			if (!rs.getBoolean(4)) {
				...
				columnsizeMap.put(rs.getString(2), rs.getInt(6));
		...
		columnsizes.setSize(nameMap.size());
		...
		for (Map.Entry<String, Integer> entry : nameMap.entrySet()) {
			...
			columnsizes.set(value.intValue(),columnsizeMap.get(key));
		}
	...
	public VerticaRecord1(Configuration conf,Text tableName) 
			throws ClassNotFoundException, SQLException, IOException {
	 ...
	 * @throws SQLException
	*/
	public VerticaRecord1(ResultSet results) throws SQLException {
	...
	//added at 2015/1/12 to get column names only
	...
	//check the value confirm to the type or not 
	public boolean checkValueType(int index,String s)
	{
	...
	//check the column size //evan add 
	public boolean set2(int i, Object value) throws IOException{
		if (i >= names.size())
			throw new IndexOutOfBoundsException("Index " + i
					+ " greater than input size " + values.size());
		
		if(columnsizes.get(i)>=value.toString().getBytes("UTF-8").length)
			{
			try
			{
			  values.set(i, value);
			...
			//truncate the rows
		{
			try
			{
				values.set(i, value.toString().substring(0,columnsizes.get(i)));
			}
			catch(Exception e)
			{}
			return false;
...
/*		else if (type == VerticaDayTimeInterval.INTERVAL_DAY
	...
	public void write(PreparedStatement stmt) throws SQLException {
		for (int i = 0; i < columns; i++) {
			...
			//if( stmt.getParameterMetaData().getPrecision(oneBasedIndex)>=obj.toString().getBytes().length)
